/**
 * CharacterMovement:
 * Controla el movimiento del personaje en el Hub 3D
...
export class CharacterMovement {
    private character: StickmanCharacter;          // referencia al personaje q vamos a mover
    private cameraController: CameraController;    // controlador de camara pa calcular direcciones relativas
    private inputHandler: InputHandler;            // sist q detecta q teclas estan presionadas
    ...
    /**
     * Constructor: inicializa el sist de movimiento del personaje
     * @param character -> personaje q vamos a controlar
     * @param cameraController -> camara pa calcular direcciones
     * @param inputHandler -> sist de input de teclado
     * @param initialPosition -> pos inicial del personaje (x defecto es el origen 0,0,0)
     */
    constructor(
    ...
    /**
     * Actualiza el movimiento del personaje cada frame
     * este metodo se llama continuamente desde el game loop principal
     * calcula la direccion de movimiento, anima, rota y mueve al personaje
     * @param moveSpeed -> velocidad de movimiento (x defecto 0.3)
     * @returns true si se presiono alguna tecla, false si no
     */
    public update(moveSpeed: number = 0.3): boolean {
        ...
        if (camera) {
            ...
            if (this.inputHandler.isKeyPressed('ArrowUp')) {
                // addInPlace() -> suma el vector al targetPosition directamente
                // scale() -> multiplica el vector x un numero (moveSpeed)
                this.targetPosition.addInPlace(forward.scale(moveSpeed));
                keyPressed = true;  // marcamos q hubo input
            }
            // Flecha abajo -> mover hacia atras (opuesto a la direccion de camara)
            if (this.inputHandler.isKeyPressed('ArrowDown')) {
                // -moveSpeed -> movimiento en direccion opuesta
                this.targetPosition.addInPlace(forward.scale(-moveSpeed));
                keyPressed = true;
            }
        ...
        this.targetPosition.x = Math.max(-55, Math.min(55, this.targetPosition.x));
        // Math.max() -> devuelve el mayor de dos numeros
        // Math.min() -> devuelve el menor de dos numeros
        // esto "clampea" el valor entre -55 y 55
        this.targetPosition.z = Math.max(-55, Math.min(55, this.targetPosition.z));

        const current = this.character.getPosition();  // pos actual del personaje
        const lerpSpeed = 0.25;                        // velocidad de interpolacion (0 a 1)
        ...
        if (keyPressed && camera) {
            ...
            if (this.inputHandler.isKeyPressed('ArrowUp')) moveDir.addInPlace(forward);
            if (this.inputHandler.isKeyPressed('ArrowDown')) moveDir.addInPlace(forward.scale(-1));
            if (this.inputHandler.isKeyPressed('ArrowLeft')) moveDir.addInPlace(right);
            if (this.inputHandler.isKeyPressed('ArrowRight')) moveDir.addInPlace(right.scale(-1));
    ...
    /**
     * Devuelve la pos objetivo actual del personaje
     * util pa debugging o pa q otros sist sepan hacia donde se mueve
     * @returns Vector3 con la pos objetivo
     */
