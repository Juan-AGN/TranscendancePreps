/**
 * Arcade modelo glb maquina arcade retro
...
// Clase molde pa crear la maquina arcade en el escenario {obj decorativo}
export class Arcade {
    ...
    private readonly targetPosition: Vector3;             // pos objetivo de la arcade
    ...
    /** Carga el model GLB d la arcade d forma asincrona ste metodo es PRIVADO, se ejecuta automatic
      en el constructor * aplica transformaciones (pos, escala, rotacion) y configura sombras */
    private async load(): Promise<void> {
        // private: solo se puede llamar desde dentro de Arcade(lo llama el constructor)
        ...
        try {
            ...
            if (result.meshes.length === 0) {
                console.warn('Arcade: no se cargaron meshes');
                return;
            }
            //debug para ver cuantos elem carga babylon
            console.log(`Arcade: ${result.meshes.length} meshes totales`);
            ...
            console.log('Arcade cargado en:', rootMesh.position.toString());
            ...
            console.log(`Arcade: ${this.meshes.length} meshes configurados`);
        } catch (error) {
